...
import { AlertCircle, CheckCircle, FileImage, FolderOpen, Loader2, Upload, X } from "lucide-react";
import { useRouter } from "next/navigation";
import { useCallback, useState } from "react";
import { checkFileSystemSupport } from "@/lib/fs/file-system-access";
import { type ImportProgress, importer } from "@/lib/fs/importer";
          ...
          <div className="bg-destructive/10 border border-destructive rounded-lg p-4">
            <div className="flex items-center space-x-2">
              ...
              <p className="text-destructive">
                Your browser does not support the File System Access API. Please use a modern browser like Chrome, Edge,
                or Brave.
              </p>
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              {progress.failed > 0 && (
                <div className="bg-destructive/10 border border-destructive rounded-lg p-3">
                  <p className="text-sm text-destructive">{progress.failed} file(s) failed to import</p>
                </div>
              )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-32 overflow-y-auto space-y-1">
                    {progress.errors.map((err) => (
                      <div key={`${err.file}:${err.error}`} className="text-xs p-2 bg-muted rounded">
