...
import { useFilteredImages, useImages } from "@/lib/store/hooks";
import { ImageAdjustments, type UIState, type ViewMode } from "@/lib/types";
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Import some photos first to start culling</p>
          <button
        ...
        <div className="flex items-center gap-4">
          {currentImage && (
            <>
              ...
              <RatingControls image={currentImage} onUpdate={forceUpdate} />
            </>
          )}
        </div>
      </div>
              ...
              <ImageViewer
                ...
                onToggleCropTool={
                  image.id === uiState.currentImageId
                    ? () => setUIState((prev) => ({ ...prev, showCropTool: !prev.showCropTool }))
                    : undefined
                }
                onToggleAdjustments={
                  image.id === uiState.currentImageId
              ...
              >
                ...
                {image.rating > 0 && (
                  <div className="absolute bottom-1 left-1 flex">
                    {Array.from({ length: Math.min(5, image.rating) }, (_, star) => star + 1).map((star) => (
                      <div key={star} className="w-2 h-2 bg-yellow-400 rounded-full mr-0.5" />
                    ))}
                  </div>
                )}
