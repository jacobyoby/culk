import { type RawProcessingResult, RawProcessor, type RawProcessorOptions } from "../lib/raw/processor";
...
self.onmessage = async (event: MessageEvent<RawWorkerMessage>) => {
  ...
  try {
    ...
    switch (type) {
      ...
      case "preview":
        result = await processor.generatePreview(fileBuffer, maxWidth || 1920, maxHeight || 1080);
        break;

      case "thumbnail":
        result = await processor.generateThumbnail(fileBuffer, width || 200, height || 200);
        break;
