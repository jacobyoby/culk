...
import { Check, Crop, Maximize2, RotateCw, Sliders, ZoomIn, ZoomOut } from "lucide-react";
...
import { formatAperture, formatExposureTime, formatFocalLength, formatISO } from "@/lib/utils/exif";
...
export function ImageViewer({
  ...
  const {
    getAdjustments,
    setAdjustments: setContextAdjustments,
    getActivePreset,
    setActivePreset,
  } = useImageAdjustments();
  ...
  return (
    <div className={`relative bg-black overflow-hidden ${className}`} ref={containerRef}>
      ...
      <div className="absolute top-4 right-4 z-10 flex gap-2">
        ...
        />
        <IconButton icon={RotateCw} onClick={rotate} variant="glass" tooltip="Rotate" />
        <IconButton icon={Maximize2} onClick={resetView} variant="glass" tooltip="Reset View" />
        <IconButton
          icon={Crop}
        ...
        <IconButton
          ...
          tooltip={
            Object.values(adjustments).some((val) => val !== 0)
              ? "Adjustments Applied - Click to Modify"
                ...
                {image.faces.map((face) => (
                  <g key={face.id}>
                    <rect
                      ...
                      fill="none"
                      stroke={
                        face.eyeState?.left === "closed" || face.eyeState?.right === "closed" ? "#ef4444" : "#10b981"
                      }
                      strokeWidth="2"
                    />
                    {face.eyeState && (
      ...
      {showMetadata && (
        <div className="absolute bottom-4 left-4 bg-black/70 rounded-lg p-3 text-white text-sm">
          ...
          {image.metadata.dateTime && <p>{new Date(image.metadata.dateTime).toLocaleString()}</p>}
