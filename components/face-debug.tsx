  ...
  return (
    <div className="p-4 bg-gray-100 rounded-lg">
      ...
      <div className="mb-4">
        ...
        <p>
          <strong>Images with faces:</strong> {images.filter((img) => img.faces && img.faces.length > 0).length}
        </p>
        <p>
          <strong>Total faces found:</strong> {images.reduce((sum, img) => sum + (img.faces?.length || 0), 0)}
        </p>
      </div>
      ...
      {selectedImageData && (
        <div className="bg-white p-4 rounded border">
          ...
          <div className="text-sm space-y-1">
            <p>
              <strong>ID:</strong> {selectedImageData.id}
            </p>
            <p>
              <strong>File Size:</strong> {(selectedImageData.fileSize / 1024).toFixed(1)} KB
            </p>
            <p>
              <strong>Has Thumbnail:</strong> {selectedImageData.thumbnailDataUrl ? "Yes" : "No"}
            </p>
            <p>
              <strong>Has Preview:</strong> {selectedImageData.previewDataUrl ? "Yes" : "No"}
            </p>
            <p>
              <strong>Faces Data:</strong> {selectedImageData.faces ? "Present" : "Missing"}
            </p>
            <p>
              <strong>Face Count:</strong> {selectedImageData.faces?.length || 0}
            </p>
          </div>
            ...
            <div className="mt-4">
              ...
              {selectedImageData.faces.map((face, index) => (
                <div key={face.id} className="bg-gray-50 p-2 rounded mb-2 text-xs">
                  ...
                  <p>
                    Position: ({face.bbox.x.toFixed(1)}%, {face.bbox.y.toFixed(1)}%)
                  </p>
                  <p>
                    Size: {face.bbox.width.toFixed(1)}% × {face.bbox.height.toFixed(1)}%
                  </p>
                  <p>
                    Eyes: L:{face.eyeState?.left || "unknown"}, R:{face.eyeState?.right || "unknown"}
                  </p>
          ...
          {(!selectedImageData.faces || selectedImageData.faces.length === 0) && (
            <div className="mt-4 p-3 bg-yellow-100 rounded">
              <p className="text-sm text-yellow-800">No faces detected for this image. This could mean:</p>
      ...
      {images.length === 0 && (
        <div className="p-3 bg-blue-100 rounded">
          <p className="text-sm text-blue-800">No images found. Import some photos to test face detection.</p>
        </div>
      )}
