...
import { canvasToBlob, createCanvasFromImage, downloadBlob, getImageData } from "@/lib/utils/canvas";
import { applyCrop, type CropRegion, detectAutoCropRegion } from "@/lib/utils/crop";
import { withFileErrorHandling, withImageProcessingErrorHandling } from "@/lib/utils/error-handling";
import { generateFaceAwareCrop, getFacesForImage } from "@/lib/utils/face-detection";
...
export function CropTool({ image, isOpen, onClose, onCropApplied, onImageUpdate }: CropToolProps) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: loadImage is recreated each render; reload only when the dialog or preview changes
  useEffect(() => {
  ...
  const generateAutoCropSuggestions = async (imageData: ImageData) => {
    ...
    try {
      ...
      if (faces && faces.length > 0) {
        const faceAwareCrop = generateFaceAwareCrop(imageData.width, imageData.height, faces);
        suggestions.push({
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <Button onClick={onClose} variant="ghost" size="icon" className="hover:bg-muted">
            ×
          </Button>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="space-y-2">
                {autoCropSuggestions.map((suggestion) => (
                  <button
                    key={`${suggestion.method}-${suggestion.confidence}`}
                    onClick={() => applySuggestion(suggestion)}
                    className="w-full text-left p-3 rounded-lg border border-border hover:bg-muted transition-colors"
                  >
                    <div className="font-medium capitalize">{suggestion.method.replace("-", " ")}</div>
            ...
            {cropRegion && (
              <div className="p-3 bg-muted rounded-lg">
                ...
                <div className="text-sm space-y-1">
                  <div>
                    Position: {cropRegion.x}, {cropRegion.y}
                  </div>
                  <div>
                    Size: {cropRegion.width} × {cropRegion.height}
                  </div>
                  <div>Aspect: {(cropRegion.width / cropRegion.height).toFixed(2)}:1</div>
                </div>
              </div>
            )}
