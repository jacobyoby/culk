...
import { MessageType, type StatusMessage } from "@/lib/utils/processing-state";
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          {message.title && <p className={clsx("text-sm font-medium mb-1", config.titleColor)}>{message.title}</p>}
          <p className={clsx("text-sm", config.messageColor)}>{message.message}</p>
          {message.details && <p className={clsx("text-xs mt-1", config.detailsColor)}>✓ {message.details}</p>}
        </div>
        {onDismiss && (
          <button
            onClick={onDismiss}
            className={clsx("flex-shrink-0 p-1 rounded hover:bg-black/20 transition-colors", config.iconColor)}
            aria-label="Dismiss"
          >
            ×
          </button>
...
export function SuccessMessage({ children, className }: { children: React.ReactNode; className?: string }) {
  return <StatusMessageComponent message={{ type: "success", message: children as string }} className={className} />;
}

export function ErrorMessage({ children, className }: { children: React.ReactNode; className?: string }) {
  return <StatusMessageComponent message={{ type: "error", message: children as string }} className={className} />;
}

export function InfoMessage({ children, className }: { children: React.ReactNode; className?: string }) {
  return <StatusMessageComponent message={{ type: "info", message: children as string }} className={className} />;
}

export function WarningMessage({ children, className }: { children: React.ReactNode; className?: string }) {
  return <StatusMessageComponent message={{ type: "warning", message: children as string }} className={className} />;
}
