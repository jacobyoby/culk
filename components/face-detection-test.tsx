...
export function FaceDetectionTest() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: detectFacesBySkinTone is a local helper defined below this callback
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div className="relative border border-gray-300 rounded-lg overflow-hidden">
            <canvas ref={canvasRef} className="max-w-full" width={300} height={200} />
        ...
        <div className="space-y-4">
          ...
          {results && (
            <div className="bg-gray-50 p-4 rounded-lg">
              <div
                className={`p-3 rounded mb-3 ${results.faces.length > 0 ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}
              >
                <strong>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div>
                  <strong>Processing time:</strong> {results.processingTime.toFixed(2)}ms
                </div>
                {results.error && (
              ...
              {results.faces.map((face, index) => (
                <div key={face.id || index} className="mt-3 p-3 bg-white rounded border">
                  ...
                  <div className="text-sm space-y-1 mt-2">
                    ...
                    <div>
                      <strong>Position:</strong> ({face.bbox.x.toFixed(1)}%, {face.bbox.y.toFixed(1)}%)
                    </div>
                    <div>
                      <strong>Size:</strong> {face.bbox.width.toFixed(1)}% × {face.bbox.height.toFixed(1)}%
                    </div>
                    {face.eyeState && (
                      <div>
                        <strong>Eyes:</strong> L:{face.eyeState.left}, R:{face.eyeState.right}
                      </div>
                    )}
                    {typeof face.focusScore === "number" && (
                      <div>
                        <strong>Focus score:</strong> {face.focusScore.toFixed(2)}
                      </div>
                    )}
                  </div>
                </div>
