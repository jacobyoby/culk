...
function isSkinToneMultiSpace(r, g, b) {
  ...
  const ycbcrSkin = Y >= 50 && Y <= 235 && Cb >= 85 && Cb <= 135 && Cr >= 135 && Cr <= 180;

  // Alternative YCbCr range for different skin tones
  const ycbcrSkin2 = Y >= 60 && Y <= 230 && Cb >= 77 && Cb <= 127 && Cr >= 133 && Cr <= 173;
  ...
  const rgbSkin1 = r > 95 && g > 40 && b > 20 && r > g && r > b && Math.abs(r - g) > 15 && r - b > 15;

  // Lighter skin tones
  const rgbSkin2 = r > 200 && g > 210 && b > 170 && Math.abs(r - g) < 15 && r > b && g > b;
...
function clusterSkinPixels(skinPixels, imageData) {
  ...
  for (const pixel of skinPixels) {
    ...
    if (cluster.length >= 100) {
      // More substantial minimum for face regions
      ...
      const isValidSize =
        clusterWidth >= minSize && clusterHeight >= minSize && clusterWidth <= maxSize && clusterHeight <= maxSize;
      ...
      console.log(
        `Cluster: ${clusterWidth.toFixed(1)}x${clusterHeight.toFixed(1)}, ratio: ${aspectRatio.toFixed(2)}, face score: ${faceScore.toFixed(3)}, pixels: ${cluster.length}`,
      );
      ...
      if (isValidSize && isValidAspectRatio && faceScore > 0.3) {
        ...
        console.log(
          `Rejected cluster: size=${isValidSize}, ratio=${isValidAspectRatio}, faceScore=${faceScore.toFixed(3)} < 0.3`,
        );
      }
    }
  }
  ...
  const faces = clusters
    .map((cluster, index) => {
      ...
      return {
        ...
        confidence: Math.min(
          0.85,
          (cluster.length / 100) * (finalAspectRatio >= 0.7 && finalAspectRatio <= 1.5 ? 1.2 : 0.8),
        ), // Boost confidence for face-like ratios
    ...
    .filter((face) => {
      ...
      console.log(
        `Face area ratio: ${areaRatio.toFixed(3)} (${face.bbox.width.toFixed(1)}% x ${face.bbox.height.toFixed(1)}%)`,
      );
...
function analyzeEyeStateHeuristic(imageData, face) {
  ...
  const leftEyeState = analyzeEyeRegionHeuristic(data, width, height, leftEyeX, eyeRegionY, eyeWidth, eyeRegionHeight);
