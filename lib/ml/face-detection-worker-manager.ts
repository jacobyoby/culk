...
export class FaceDetectionWorkerManager {
  ...
  async initialize(options: { faceConfidenceThreshold?: number } = {}): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Face detection worker initialization failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private detectFacesBySkinTone(imageData: ImageData): FaceDetection[] {
    ...
    if (faceWidth < minSize || faceHeight < minSize || faceWidth > maxSize || faceHeight > maxSize) {
  ...
  async updateSettings(settings: { faceConfidenceThreshold?: number; eyeConfidenceThreshold?: number }): Promise<void> {
    if (!this.isInitialized) return;
