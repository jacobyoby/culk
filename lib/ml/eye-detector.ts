...
import type { EyeState, FaceDetection } from "../types";
...
export class EyeDetector {
  ...
  async analyzeEyeState(imageData: ImageData, faces: FaceDetection[]): Promise<FaceDetection[]> {
  ...
  private async detectEyeStateForFace(imageData: ImageData, face: FaceDetection): Promise<EyeState> {
    if (!this.session) {
