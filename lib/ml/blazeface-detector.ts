...
export class BlazeFaceDetector {
  ...
  async detectFaces(imageData: ImageData, options: BlazeFaceOptions = {}): Promise<FaceDetection[]> {
    if (!this.isInitialized || !this.session) {
      await this.initialize();
    }

    const { confidenceThreshold = 0.7, maxFaces = 10 } = options;
    ...
    try {
      ...
      const faces = this.postprocessResults(results, confidenceThreshold, maxFaces);
  ...
  private preprocessImage(imageData: ImageData): ort.Tensor {
    ...
    ctx.drawImage(this.imageDataToCanvas(imageData), 0, 0, width, height, 0, 0, targetSize, targetSize);
  ...
  private postprocessResults(
    ...
    try {
      ...
      for (let i = 0; i < Math.min(numDetections, maxFaces); i++) {
        ...
        console.log(
          `Detection ${i}: bbox=[${xmin.toFixed(3)}, ${ymin.toFixed(3)}, ${xmax.toFixed(3)}, ${ymax.toFixed(3)}]`,
        );
