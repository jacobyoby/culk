...
import { type EyeDetector, eyeDetector } from "./eye-detector";
import { type DetectionResult, type FaceDetector, faceDetector } from "./face-detector";
...
export class FaceDetectionManager {
  ...
  async detectFaces(imageData: ImageData, options: FaceDetectionOptions = {}): Promise<FaceAnalysisResult> {
    ...
    const { includeEyeState = true, calculateFocusScore = true } = options;
    ...
    for (const pixel of skinPixels) {
      ...
      while (queue.length > 0) {
        ...
        for (const candidate of skinPixels) {
          ...
          const distance = Math.sqrt((current.x - candidate.x) ** 2 + (current.y - candidate.y) ** 2);
  ...
  updateSettings(settings: { faceConfidenceThreshold?: number; eyeConfidenceThreshold?: number }): void {
  ...
  async dispose(): Promise<void> {
    await Promise.all([this.faceDetector.dispose(), this.eyeDetector.dispose()]);
  }
