...
import { type GroupRec, type ImageRec, ImportSession, type ProjectMeta } from "../types";
import { type XMPExportOptions, xmpWriter } from "../xmp/writer";
...
export class SessionExporter {
  ...
  private generateCSVExport(images: ImageRec[], includeMetadata: boolean): string {
    ...
    const rows = images.map((image) => {
      ...
      if (includeMetadata) {
        const eyesOpenCount =
          image.faces?.filter((face) => face.eyeState?.left === "open" && face.eyeState?.right === "open").length || 0;
  ...
  async getExportPreview(options: ExportOptions = {}): Promise<{
    ...
    const estimatedSize = result.files.reduce((total, file) => total + new Blob([file.content]).size, 0);
