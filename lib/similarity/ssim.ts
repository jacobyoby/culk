...
export function calculateSSIM(img1Data: ImageData, img2Data: ImageData, windowSize: number = 11): SSIMResult {
...
function extractWindow(data: number[], width: number, centerX: number, centerY: number, windowSize: number): number[] {
  const window = [];
  const halfWindow = Math.floor(windowSize / 2);
...
export async function calculateSSIMFromFiles(file1: File, file2: File): Promise<SSIMResult> {
  const [img1Data, img2Data] = await Promise.all([loadImageData(file1), loadImageData(file2)]);
