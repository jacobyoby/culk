...
import type { GroupRec, ImageRec, ImportSession, ProjectMeta } from "../types";
...
export class PhotoCullDB extends Dexie {
  ...
  async clearAll() {
    await this.transaction("rw", this.images, this.groups, this.projects, this.sessions, async () => {
      await Promise.all([this.images.clear(), this.groups.clear(), this.projects.clear(), this.sessions.clear()]);
    });
  }
