...
import { GroupRec, type ImageRec, ImportSession, ProjectMeta } from "../types";
import { db } from "./db";
...
export function useImagesByGroup(groupId: string | undefined) {
  return useLiveQuery(() => (groupId ? db.getImagesByGroup(groupId) : []), [groupId]) ?? [];
}

export function useImagesByRating(rating: number) {
  return useLiveQuery(() => db.getImagesByRating(rating), [rating]) ?? [];
}

export function useImagesByFlag(flag: "pick" | "reject" | null) {
  return useLiveQuery(() => (flag ? db.getImagesByFlag(flag) : []), [flag]) ?? [];
}
...
export function useGroup(id: string | undefined) {
  return useLiveQuery(() => (id ? db.groups.get(id) : undefined), [id]);
}
  ...
  return (
    useLiveQuery(async () => {
      ...
      switch (filter) {
        ...
        case "blurry":
          return images.filter((img) => img.blurScore !== undefined && img.blurScore > 100);
        case "eyes-closed":
          return images.filter((img) =>
            img.faces?.some((face) => face.eyeState?.left === "closed" || face.eyeState?.right === "closed"),
          );
        default:
          return images;
      }
