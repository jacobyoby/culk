...
export function formatEnhancementMessage(result: EnhancementResult): StatusMessage {
  if (!result.applied) {
    return createStatusMessage("info", "No Enhancement Needed", "Image appears well-exposed already");
  }
...
export function formatPresetMessage(presetName: string): StatusMessage {
  return createStatusMessage("success", `Applied ${presetName} preset`, "Style applied to this image only");
}
