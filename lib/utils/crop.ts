...
export function detectAutoCropRegion(
  ...
  const { method = "edge-detection", padding = 0.05, minCropRatio = 0.7 } = options;
...
function detectEdgeBasedCrop(imageData: ImageData, padding: number, minCropRatio: number): AutoCropResult {
...
function detectContentAwareCrop(imageData: ImageData, padding: number, minCropRatio: number): AutoCropResult {
  const { data, width, height } = imageData;
  ...
  const combinedMap = combineContentMaps({ edgeMap, colorMap, saliencyMap, faceHeuristics }, width, height);
...
function detectGoldenRatioCrop(imageData: ImageData, padding: number): AutoCropResult {
  const { width, height } = imageData;
...
function detectCenterCrop(imageData: ImageData, padding: number): AutoCropResult {
  const { width, height } = imageData;
...
function calculateInterestMap(data: Uint8ClampedArray, width: number, height: number): number[] {
...
function combineContentMaps(
  ...
  for (let i = 0; i < combined.length; i++) {
    combined[i] =
      ...
      normColor[i] * 0.2 + // Color variance
      normSaliency[i] * 0.3 + // Saliency (center bias + contrast)
...
export async function applyCrop(imageFile: File, cropRegion: CropRegion): Promise<Blob> {
