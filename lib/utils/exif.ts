...
export async function extractMetadata(file: File | Blob): Promise<ImageMetadata> {
  try {
      ...
      if (tags[tagName] && !apertureResult) {
        ...
        if (apertureTag.description) {
          ...
          if (descMatch) {
            ...
            if (!isNaN(descValue) && descValue > 0 && descValue < 100) {
              // reasonable aperture range
        ...
        if (!apertureResult && apertureTag.value !== undefined) {
          ...
          if (!isNaN(apertureNum) && apertureNum > 0) {
            ...
            } else if (apertureNum > 22) {
              // f/22 is common max aperture
            ...
            if (scaledValue >= 1.0 && scaledValue <= 22.0) {
              ...
              console.log(
                `Aperture from ${tagName} raw value (scaled ${apertureNum} -> ${scaledValue}):`,
                apertureResult,
              );
              break;
            } else {
    ...
    if (tags.GPSLatitude && tags.GPSLatitudeRef) {
      metadata.gpsLatitude = convertDMSToDD(tags.GPSLatitude.description, tags.GPSLatitudeRef.value as string);
    }

    if (tags.GPSLongitude && tags.GPSLongitudeRef) {
      metadata.gpsLongitude = convertDMSToDD(tags.GPSLongitude.description, tags.GPSLongitudeRef.value as string);
    }
