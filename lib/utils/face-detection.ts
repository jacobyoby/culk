...
export async function getFacesForImage(
  ...
  const { confidenceThreshold = 0.6, useExisting = true, forceRedetect = false } = options;
...
export function hasClosedEyes(image: ImageRec): boolean {
  return image.faces?.some((face) => face.eyeState?.left === "closed" || face.eyeState?.right === "closed") ?? false;
}
...
export function getFaceDetectionStatus(image: ImageRec): {
  ...
  const hasOpenEyesFaces = image.faces!.some(
    (face) => face.eyeState?.left === "open" && face.eyeState?.right === "open",
  );
...
export function compareFaceQuality(a: ImageRec, b: ImageRec): number {
  ...
  if (aStatus.hasFaces && bStatus.hasFaces) {
    if (aStatus.hasOpenEyes && !aStatus.hasClosedEyes && (!bStatus.hasOpenEyes || bStatus.hasClosedEyes)) return -1;
    if ((!aStatus.hasOpenEyes || aStatus.hasClosedEyes) && bStatus.hasOpenEyes && !bStatus.hasClosedEyes) return 1;
...
export async function batchDetectFaces(
  ...
  const imagesToProcess = images.filter((img) => options.forceRedetect || needsFaceDetection(img));
