import type { ImageAdjustments, ImageMetadata } from "../types";
...
export function analyzeExposure(histogram: HistogramData, metadata?: ImageMetadata): ExposureAnalysis {
  ...
  const exposureScore =
    (meanLuminance - EXPOSURE_THRESHOLDS.IDEAL_MEAN_LUMINANCE) / EXPOSURE_THRESHOLDS.IDEAL_MEAN_LUMINANCE;

  // Determine exposure state
  const isUnderexposed =
    meanLuminance < EXPOSURE_THRESHOLDS.UNDEREXPOSED_MEAN ||
    shadowPercent > EXPOSURE_THRESHOLDS.SHADOW_CLIPPING_PERCENT;
  const isOverexposed =
    meanLuminance > EXPOSURE_THRESHOLDS.OVEREXPOSED_MEAN ||
...
export function autoEnhanceImage(
  ...
  if (analysis.shadowClipping > 10) {
    // Lift shadows
    const maxShadowLift = conservative
      ? ENHANCEMENT_LIMITS.MAX_SHADOW_LIFT_CONSERVATIVE
      : ENHANCEMENT_LIMITS.MAX_SHADOW_LIFT_NORMAL;
  ...
  if (analysis.highlightClipping > 3) {
    // Recover highlights
    const maxHighlightRecovery = conservative
      ? ENHANCEMENT_LIMITS.MAX_HIGHLIGHT_RECOVERY_CONSERVATIVE
      : ENHANCEMENT_LIMITS.MAX_HIGHLIGHT_RECOVERY_NORMAL;
  ...
  if (analysis.isUnderexposed) {
    const maxBrightnessBump = conservative
      ? ENHANCEMENT_LIMITS.MAX_BRIGHTNESS_BOOST_CONSERVATIVE
      : ENHANCEMENT_LIMITS.MAX_BRIGHTNESS_BOOST_NORMAL;
    ...
    const maxBrightnessReduction = conservative
      ? ENHANCEMENT_LIMITS.MAX_BRIGHTNESS_REDUCTION_CONSERVATIVE
      : ENHANCEMENT_LIMITS.MAX_BRIGHTNESS_REDUCTION_NORMAL;
  ...
  if (analysis.dynamicRange < EXPOSURE_THRESHOLDS.LOW_DYNAMIC_RANGE && !analysis.isOverexposed) {
    ...
    const maxContrastBoost = conservative
      ? ENHANCEMENT_LIMITS.MAX_CONTRAST_BOOST_CONSERVATIVE
      : ENHANCEMENT_LIMITS.MAX_CONTRAST_BOOST_NORMAL;
    const contrastBoost = Math.min(
      maxContrastBoost,
      (EXPOSURE_THRESHOLDS.LOW_DYNAMIC_RANGE - analysis.dynamicRange) / 8,
    );
    adjustments.contrast = contrastBoost;
    confidence += 0.1;
  } else if (
    analysis.dynamicRange > EXPOSURE_THRESHOLDS.HIGH_DYNAMIC_RANGE &&
    (analysis.isUnderexposed || analysis.isOverexposed)
  ) {
    // High dynamic range with exposure issues, reduce contrast slightly
    adjustments.contrast = conservative
      ? ENHANCEMENT_LIMITS.CONTRAST_REDUCTION_CONSERVATIVE
      : ENHANCEMENT_LIMITS.CONTRAST_REDUCTION_NORMAL;
  }
  ...
  if (analysis.midtoneBalance > EXPOSURE_THRESHOLDS.GOOD_MIDTONE_BALANCE && !analysis.isOverexposed) {
    ...
    let saturationBoost = conservative
      ? ENHANCEMENT_LIMITS.SATURATION_BOOST_CONSERVATIVE
      : ENHANCEMENT_LIMITS.SATURATION_BOOST_NORMAL;
    ...
    adjustments.vibrance = conservative
      ? ENHANCEMENT_LIMITS.VIBRANCE_BOOST_CONSERVATIVE
      : ENHANCEMENT_LIMITS.VIBRANCE_BOOST_NORMAL;
    confidence += 0.1;
  }
  ...
  if (
    analysis.shadowClipping < 5 &&
    analysis.highlightClipping < 2 &&
    analysis.dynamicRange > 150 &&
    analysis.midtoneBalance > 0.3
  ) {
...
export async function autoEnhanceFromCanvas(
  ...
  try {
    ...
    throw new Error(
      `Failed to get image data for enhancement: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
