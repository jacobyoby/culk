import type { ImageRec } from "../types";
...
<x:xmpmeta xmlns:x="adobe:ns:meta/" x:xmptk="AI Photo Culling 1.0.0">
  ...
  private generateKeywords(image: ImageRec): string {
    ...
    if (image.faces && image.faces.length > 0) {
      ...
      const eyesClosedCount = image.faces.filter(
        (face) => face.eyeState?.left === "closed" || face.eyeState?.right === "closed",
      ).length;

      if (eyesClosedCount > 0) {
        keywords.push("Eyes_Closed");
      } else if (image.faces.every((face) => face.eyeState?.left === "open" && face.eyeState?.right === "open")) {
...
${faceRegions
  .map(
    (region) => `            <rdf:li rdf:parseType="Resource">
  ...
  async downloadXMPSidecars(images: ImageRec[], options: XMPExportOptions = {}): Promise<void> {
