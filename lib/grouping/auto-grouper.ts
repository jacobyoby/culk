import { areSimilar, hammingDistance } from "../similarity/phash";
import { calculateSSIM } from "../similarity/ssim";
import { db } from "../store/db";
import { type GroupRec, type ImageRec, ProjectMeta } from "../types";
...
export class AutoGrouper {
  ...
  private calculateImageScore(image: ImageRec): number {
    ...
    if (image.faces && image.faces.length > 0) {
      const eyesOpenRatio =
        image.faces.filter((face) => face.eyeState?.left === "open" && face.eyeState?.right === "open").length /
        image.faces.length;
    ...
    if (image.faces && image.faces.length > 0) {
      const avgFaceSize =
        image.faces.reduce((sum, face) => sum + face.bbox.width * face.bbox.height, 0) / image.faces.length;
