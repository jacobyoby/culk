...
describe("Docker Integration", () => {
  describe("Development Container", () => {
    ...
    it("should start dev container with hot reload", async () => {
      const { stdout } = await execAsync("docker run -d --name ai-cull-dev-test -p 3002:3000 ai-cull:dev-test");
  ...
  describe("Production Container", () => {
    it("should build production image with multi-stage", async () => {
      const { stdout, stderr } = await execAsync("docker build -t ai-cull:prod-test -f Dockerfile .");
    ...
    it("should have minimal image size", async () => {
      const { stdout } = await execAsync('docker images ai-cull:prod-test --format "{{.Size}}"');
