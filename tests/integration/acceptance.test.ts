...
describe("Acceptance Criteria", () => {
    ...
    it("should auto-group near-duplicates", () => {
      ...
      images.forEach((img) => {
        const existingGroup = groups.find((g) => {
          ...
          if (!rep) return false;
          const distance = img.phash.split("").reduce((acc, bit, idx) => acc + (bit !== rep.phash[idx] ? 1 : 0), 0);
  ...
  describe("Face & Eye Detection", () => {
    ...
    it("should detect eye state correctly", () => {
      ...
      const allEyesOpen = eyeStates.filter((e) => e.leftEye === "open" && e.rightEye === "open");
...
<x:xmpmeta xmlns:x="adobe:ns:meta/">
  ...
  describe("PWA & Offline", () => {
    ...
    it("should cache app for offline use", async () => {
      const cache = {
        name: "ai-cull-v1",
        urls: ["/", "/app", "/manifest.webmanifest", "/icons/icon-192x192.png"],
      };
